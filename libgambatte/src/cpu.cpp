...
void CPU::process(unsigned long const cycles) {
	...
	while (mem_.isActive()) {
		...
		if (mem_.halted()) {
			...
			//if(first-- >= 0)
				//std::cout << "PC2: " << std::hex << PC << std::endl;
			//if(PC == 0x40)
				//std::cout << "VBLANK" << std::endl;

			int FullPC = pc;
			if(pc >= 0xFE00) FullPC = -1;
			else if(pc >= 0x8000) FullPC = pc;
			else if(pc >= 0x4000) FullPC += (mem_.cart_.memptrs_.curRomBank-1)*0x4000;

			//if(FullPC == 0x9007f)
			//	std::cout << "HERE " << FullPC << std::endl;

			for(int i=0;i<numInterruptAddresses; i++) {

				if(FullPC == interruptAddresses[i]) {
					hitInterruptAddress = interruptAddresses[i];
					mem_.setEndtime(cycleCounter, 0); // set end time to NOW
					//std::cout << "hit interrupt address " << std::hex << interruptAddresses[i] << std::endl;
					break;
				}
			}
			if(hitInterruptAddress) // leave if hit interrupt
				continue;
			PC_READ(opcode);
