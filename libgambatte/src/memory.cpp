				...
				while (length--) {
					...
					if (cc - 3 > lOamDmaUpdate) {
						...
						if (oamDmaPos_ < 0xA0) {
							...
							#ifdef GAMBATTELOG
							log_write(0xfe00 | (src & 0xFF), data, cc);
							#endif
		...
		if (oamDmaPos_ < 0xA0) {
			if (oamDmaPos_ == 0)
				startOamDma(lastOamDmaUpdate_ - 1);
                        #ifdef GAMBATTELOG
			log_write(0xfe00 | oamDmaPos_, oamDmaSrc ? oamDmaSrc[oamDmaPos_] : cart_.rtcRead(), lastOamDmaUpdate_);
			#endif
...
void Memory::log_speedChange(const unsigned long cycleCounter) {
	...
	int frameCycles = lcd_.lyCounter().frameCycles(cycleCounter) << oldSpeed;
	...
	curFrameStartCycle = ((long)cycleCounter) - (lcd_.lyCounter().frameCycles(cycleCounter) << newSpeed);
	curFrame = frame;
...
void Memory::log_write(const unsigned P, const unsigned data, const unsigned long cycleCounter) {
	...
	int frameCycles = lcd_.lyCounter().frameCycles(cycleCounter) << isDoubleSpeed();
	long frameStartCycle = ((long)cycleCounter) - frameCycles;
...
void Memory::loadOrSave(loadsave& state)
{
	...
	//std::cout << "y " << state.getOffset() << std::endl;
	state(divLastUpdate_);
	//std::cout << "z " << state.getOffset() << std::endl;
	state(lastOamDmaUpdate_);
	//std::cout << "a " << state.getOffset() << std::endl;
	intreq_.loadOrSave(state);
	//std::cout << "b " << state.getOffset() << std::endl;
	cart_.loadOrSave(state);
	//std::cout << "c " << state.getOffset() << std::endl;
	tima_.loadOrSave(state);
	//std::cout << "d " << state.getOffset() << std::endl;
	lcd_.loadOrSave(state);
	//std::cout << "e " << state.getOffset() << std::endl;
	psg_.loadOrSave(state);
	//std::cout << "f " << state.getOffset() << std::endl;
