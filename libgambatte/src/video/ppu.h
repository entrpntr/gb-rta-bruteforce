...
public:
	...
	void setFbline(unsigned ly) { fbline_ = buf_ ? buf_ + std::ptrdiff_t(ly) * pitch_ : nullfbline(); }
		...
		bool var;
		//std::cout << "d1a " << state.getOffset() << std::endl;
		var = (buf_ != 0);
		state(var);
		if(var)
			state(buf_, 160*144);
		//std::cout << "d1b " << state.getOffset() << std::endl;
		var = (fbline_ != nullfbline());
		state(var);
		//std::cout << "d1c " << state.getOffset() << std::endl;
		if(var) {
			...
			fbline_ = nullfbline();
		//std::cout << "d1d " << state.getOffset() << std::endl;
	}
